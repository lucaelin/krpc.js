...
class Service {
    ...
    stream(name, onChange) {
        // if stream was created before, call onChange to match behaviour with the first call
        if (typeof onChange === 'function' && typeof this._streamCache[name] !== 'undefined') {
            onChange(this._streamCache[name]);
        }
