...
function decodeSInt32(buffer) {
    return ByteBuffer.wrap(buffer).readVarint32ZigZag();
}
...
function decodeSInt64(buffer) {
    return ByteBuffer.wrap(buffer).readVarint64ZigZag().toNumber();
}
...
function decodeUInt32(buffer) {
    return ByteBuffer.wrap(buffer).readVarint32();
}
...
function decodeUInt64(buffer) {
    return ByteBuffer.wrap(buffer).readVarint64().toNumber();
}
