...
module.exports = class KRPC {
    ...
    connectToRPCServer() {
        return new Promise((resolve, reject)=>{
            ...
            this.rpcSocket.addEventListener('close', (event)=>console.error('TODO: rpc connection close:', event));
    ...
    async createServices() {
        ...
        let services = response.results[0].value.services.map((service)=>new Service(service, (call)=>this.send(call), (call, cb)=>this.addStream(call, cb)));
    ...
    onStreamMessage(data) {
        ...
        streamUpdate.results.forEach((update)=>{
            ...
            stream.callbacks.forEach((f)=>f(decodedValue));
        });
        // return this.stream.emitter.emit('message', streamUpdate, data);
    }
    ...
    registerStream(procedure, stream, onUpdate, onDelete) {
        ...
        if (!s) {
            s = {
                ...
                id: stream.id,
                remove: async ()=>{
                    await this.removeStream(s);
                    s.callbacks = [];
                    s.removeCallbacks.forEach((f)=>f());
                    s.removeCallbacks = [];
                }
        ...
        return {
            ...
            handler: onUpdate,
            remove: ()=>{
                s.callbacks = s.callbacks.filter((i)=>i!==onUpdate);
                if (!s.callbacks.lenth) {s.remove();}
            }
        };
    }
